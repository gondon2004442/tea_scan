...
const teaImagesOutPath = path.join(process.cwd(), 'src', 'data', 'tea-images.generated.ts');
const unmatchedPath = path.join(process.cwd(), 'scripts', 'unmatched-teas.json');
const teaImagesDir = path.join(process.cwd(), 'assets', 'images', 'teas');
const teaImagesManifestPath = path.join(teaImagesDir, 'manifest.json');

const TEA_IMAGE_SIZE = 400;
const TEA_IMAGE_WEBP_QUALITY = 80;
...
async function fetchBufferWithRetry(url, retries = 4) {
  let lastError = null;
...
async function loadManifest() {
  try {
    const raw = await fs.readFile(teaImagesManifestPath, 'utf-8');
...
async function optimizeTeaImage(buffer) {
  return sharp(buffer)
    .rotate()
    .resize(TEA_IMAGE_SIZE, TEA_IMAGE_SIZE, { fit: 'cover', position: 'centre' })
    .webp({ quality: TEA_IMAGE_WEBP_QUALITY })
    .toBuffer();
}

async function syncTeaImages(seeds) {
  await fs.mkdir(teaImagesDir, { recursive: true });
  const manifest = await loadManifest();
  const nextManifest = { entries: {} };
  const localIds = [];
  ...
  for (const seed of seeds) {
    ...
    const fileName = `${seed.id}.webp`;
    const filePath = path.join(teaImagesDir, fileName);
    const existing = manifest[seed.id];
    const isCurrent = existing?.sourceUrl === seed.imageUrl;

    if (isCurrent) {
      try {
        await fs.access(filePath);
        nextManifest.entries[seed.id] = {
          sourceUrl: seed.imageUrl,
          updatedAt: existing.updatedAt,
        };
        localIds.push(seed.id);
        continue;
      } catch {
        // stale manifest entry — re-download below
      }
    }

    console.log(`Downloading tea image: ${seed.id}`);
    const raw = await fetchBufferWithRetry(seed.imageUrl);
    const optimized = await optimizeTeaImage(raw);
  ...
  await fs.writeFile(teaImagesManifestPath, JSON.stringify(nextManifest, null, 2), 'utf-8');

  const imageOutput = `/* AUTO-GENERATED by scripts/generate-teas.mjs. Do not edit manually. */
...
${localIds
  .map(
    (id) =>
      `  '${escape(id)}': require('../../assets/images/teas/${escape(id)}.webp'),`,
  )
  .join('\n')}
};
`;

  await fs.writeFile(teaImagesOutPath, imageOutput, 'utf-8');
...
`;
  ...
  const localImageCount = await syncTeaImages(seeds);
  ...
  console.log(`Local WebP assets: ${localImageCount}.`);
